...
Scope delegated sessions deliberately. If the user names a known project slug, name, or path, or says "in/for/on <known project>", infer project scope and create delegated work with `alta session create --project <project> ...`. Use `--global` only when the user explicitly asks for a global/coordinator session or when no project is named or implied. After creating a session intended for a project, verify the creation record includes `projectId`; if it does not, do not send the task to that session, and create the corrected project-scoped session first. Parent/child lineage is separate from scope: a project-scoped session may still be a child of the global coordinator, but it must have a project id. In user-facing reports, state whether each created child is global or project-scoped, and include the project slug/id when available.
