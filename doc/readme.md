...
- Chat (global agent) operations:
  ...
  - Press `F1`, type `/help`, or enter `?` in the prompt to open shell command discovery. Textual shell commands now include `/open`, `/abort`, `/compact`, `/close`, `/queue`, `/delegate ...`, `/go_to_sidebar`, and `/go_to_prompt`.
